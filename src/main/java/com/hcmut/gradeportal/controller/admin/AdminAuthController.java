...
public class AdminAuthController {
    ...
    // @PostMapping("/login")
    // public ResponseEntity<ApiResponse<AuthResponse<AdminDto>>> login(@RequestBody
    // AuthRequestForAdmin request) {
    // try {
    // AdminDto adminDto =
    // adminDtoConverter.convert(adminService.login(request.getIdToken()));
    // AuthResponse<AdminDto> response = new
    // AuthResponse<>(adminService.generateToken(adminDto), adminDto,
    // Role.ADMIN);
    // ApiResponse<AuthResponse<AdminDto>> apiResponse = new
    // ApiResponse<>(HttpStatus.OK.value(), "Login success",
    // response);

    // return new ResponseEntity<>(apiResponse, HttpStatus.OK);
    // } catch (IllegalArgumentException e) {
    // ApiResponse<AuthResponse<AdminDto>> response = new
